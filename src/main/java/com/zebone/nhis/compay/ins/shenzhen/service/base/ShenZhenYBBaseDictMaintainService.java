...
import com.zebone.nhis.compay.ins.shenzhen.dao.base.ShenZhenYBBaseDictMaintainMapper;
...
@Service
public class ShenZhenYBBaseDictMaintainService {
	
	@Autowired
	private ShenZhenYBBaseDictMaintainMapper szybBaseDictMainMapper;
	 ...
	 * @return
	 */
	public List<InsSzybDict> querySZYBBaseDictType(String param, IUser user){
		String euHpdicttype = JsonUtil.getFieldValue(param, "euHpdicttype");
		List<InsSzybDict> insSZybTypeList = szybBaseDictMainMapper.querySZYBBaseDictType(euHpdicttype);
	 ...
	 * @return
	 */
	public List<InsSzybDict> querySZYBBaseDictInfo(String param, IUser user){
		Map<String, Object> paramMap = JsonUtil.readValue(param, Map.class);
		List<InsSzybDict> insSZybTypeList = szybBaseDictMainMapper.querySZYBBaseDictInfo(paramMap);
		return insSZybTypeList;
	...
	public void saveSZYBType(String param, IUser user){
		InsSzybDict insSzybDict = JsonUtil.readValue(param, InsSzybDict.class);
		if (insSzybDict == null ) {
	...
	public void delSzybType(String param, IUser user) {
		...
		String upSql = "update ins_szyb_dict  set Del_flag = '1' where code_type = ?";
		String sql = "update ins_szyb_dict  set Del_flag = '1' where pk_insdict = ?";
	...
	public void batchUpdateSZYBBaseDict(String param, IUser user) {
		Map<String,Object> paramMap = JsonUtil.readValue(param, Map.class);
		List<InsSzybDict> delList = (List<InsSzybDict>) paramMap.get("delList");
		szybBaseDictMainMapper.batchUpdateSZYBBaseDict(delList);
	}
	 ...
	 * @return
	 */
	public List<InsSzybDict> saveSzybBaseDictInfo(String param, IUser user) {
		List<InsSzybDict> updateList = JsonUtil.readValue(param, new TypeReference<List<InsSzybDict>>(){});
		if(updateList ==null||updateList.size()<=0) {
			throw new BusException("未获取到要保存的信息！");
		}
    	for(InsSzybDict vo: updateList){

...
import com.zebone.nhis.compay.ins.zsrm.dao.base.ZsrmQgBaseDictMapMapper;
import com.zebone.nhis.compay.ins.zsrm.vo.base.InsQgybDictMap;
...
@Service
public class QgMediBaseDictMapService {

	@Autowired
	private ZsrmQgBaseDictMapMapper zsrmQgBaseDictMapMapper ;
	 ...
	 * @return
	 */
	public List<InsQgybDictMap> getMediType(String param, IUser user) {
		String euHpdicttype = JsonUtil.getFieldValue(param, "euHpdicttype");
		List<InsQgybDictMap> mediType = zsrmQgBaseDictMapMapper.getMediType(euHpdicttype);
		return mediType;
	...
	public void saveMediType(String param, IUser user) {
		InsQgybDictMap insSzybDictmap = JsonUtil.readValue(param, InsQgybDictMap.class);
	...
	public void delMediType(String param, IUser user) {
		...
		String upSql = "update ins_qgyb_dictmap  set Del_flag = '1' where code_type = ?";
		String sql = "update ins_qgyb_dictmap  set Del_flag = '1' where pk_insdictmap = ?";
	...
	public List<Map<String,Object>> querySZMediBaseData(String param, IUser user) {
		Map<String,Object> paramMap = JsonUtil.readValue(param, Map.class);
		List<Map<String, Object>> list = zsrmQgBaseDictMapMapper.querySZMediBaseData(paramMap);
		return list;
	}
	...
	public void batchUpdateInsSzybInfo(String param, IUser user) {
		Map<String,Object> paramMap = JsonUtil.readValue(param, Map.class);
		List<InsQgybDictMap> delList = (List<InsQgybDictMap>) paramMap.get("delList");
		zsrmQgBaseDictMapMapper.batchUpdateInsSzybInfo(delList);
	 ...
	 * @return
	 */
	public List<InsQgybDictMap> saveInsSzybDictInfo(String param, IUser user) {
		List<InsQgybDictMap> updateList = JsonUtil.readValue(param, new TypeReference<List<InsQgybDictMap>>(){});
		if(updateList ==null||updateList.size()<=0) {
			throw new BusException("未获取到要保存的信息！");
		}
    	for(InsQgybDictMap vo: updateList){

package com.zebone.nhis.compay.ins.shenzhen.service.szyb;
...
import com.zebone.nhis.common.module.compay.ins.shenzhen.szyb.InsSzybTrialstVo;
import com.zebone.nhis.common.support.CommonUtils;
import com.zebone.nhis.compay.ins.shenzhen.vo.szxnh.VisitVo;
...
@Service
public class SzybService {
	//@Autowired
	//private SzybMapper szybMapper;

	
	/**
	 * 015001011022 保存就诊登记信息数据
	 * 
	 * @param param
	 * @param user
	 * @return
	...
	public String save(String param, IUser user) {
		...
		SzybVisit visit = vo.getVisit();
		SzybVisitCity visitCity = vo.getVisitCity();
		SzybVisitXnh visitXnh = vo.getVisitXnh();
		String pkpv = "000000";
		DataBaseHelper.execute("delete from INS_SZYB_VISIT where PK_PV=?",
				new Object[] { pkpv });
		if (visit.getPvcodeIns() != null) {
			DataBaseHelper.execute(
					"delete from INS_SZYB_VISIT where PVCODE_INS=?",
	...
	//就诊登记--保存市医保
	public void savecity(SzybVisitCity visitCity, String param, IUser user) {
		User u = (User) user;
		if (visitCity != null) {
			visitCity.setPkOrg(u.getPkOrg());
	...
	//就诊登记--保存新农合
	public void savennh(SzybVisitXnh visitXnh, String param, IUser user) {
		User u = (User) user;
		if (visitXnh != null) {
	...
	public void savennh(String param, IUser user) {
		SzybVisitXnh gzybvisitxnh = JsonUtil.readValue(param,
				SzybVisitXnh.class);
		User u = (User) user;
	...
	/**
	 * 	
	 * 	015001011021 根据收费结算-住院收费明细主键的集合更新bl_ip_dt的flag_insu字段
	 */
	...
	/**
	 * 015001011024//保存预结算数据
	 * @param param
	 * @param user
	 */
	public void saveTrialInfo(String param, IUser user) {
		InsSzybTrialstVo trialstVo = JsonUtil.readValue(param,
				InsSzybTrialstVo.class);
		User u = (User) user;
		if (trialstVo != null) {
...
/*	*//**
	...
	public void updateVisit(String param, IUser user) {
		...
		if (pkvisit != null && pkpv != null) {
			DataBaseHelper
					.execute(
							"update INS_SZYB_VISIT set PK_PV=?,PK_PI=? where pk_visit=?",
							pkpv, pkpi, pkvisit);
		} else {
			throw new BusException("传入参数有空值");
		}
	...
	*//**
	...
	public void addSt(String param, IUser user) {
		// 1.1接收参数（ins_szyb_st,ins_gzyb_st_city）
		...
		// 2.2根据登记信息查询结算主键
		String qrySql = "select pk_insst from ins_szyb_st where pk_visit=? and del_flag='0' ";
		Map<String, Object> insstMap = DataBaseHelper.queryForMap(qrySql,
				new Object[] { st.getPkvisit() });

		// 2.1逻辑处理：更新结算记录-通过登记主键更新del_flag='1'
		String sql = "update ins_szyb_st set del_flag='1' where pk_visit=?";
	...
	*//**
	 * 015001007017
	 * 通过医保结算病人就诊主键PK_PV将PkSettle结算主键更新到INS_SZYB_ST表同时更新INS_SZYB_VISIT表
	 * 
	 * @param param
	 * @param user
	 *//*
	public void updatePkSettle(String param, IUser user) {
		...
		if (pkpv != null && pkSettle != null) {
			if ("1".equals(pkSettle)) {
				DataBaseHelper.execute(
						"update INS_SZYB_ST set PK_SETTLE='' where PK_PV=?",
						pkpv);
				DataBaseHelper
						.execute(
								"update INS_SZYB_VISIT set EU_STATUS_ST='0' where PK_PV=?",
								pkpv);
			} else {
				DataBaseHelper.execute(
						"update INS_SZYB_ST set PK_SETTLE=? where PK_PV=? ",
				...
				DataBaseHelper
						.execute(
								"update INS_SZYB_VISIT set EU_STATUS_ST='1' where PK_PV=? ",
								pkpv);
			}
		} else {
			throw new BusException("传入参数有空值");
	...
	*//**
	 * 015001007018通过医保结算病人就诊主键PK_PV将EU_STATUS_ST结算状态更新到INS_SZYB_VISIT表
	 * 
	 * @param param
	 * @param user
	 *//*
	public void updateEuStatusSt(String param, IUser user) {
		...
		if (pkpv != null && euStatusSt != null) {
			DataBaseHelper.execute(
					"update INS_SZYB_VISIT set EU_STATUS_ST=? where PK_PV=?",
					euStatusSt, pkpv);
		} else {
			throw new BusException("传入参数有空值");
		}
	...
	*//**
	...
	public void deletevisit(String param, IUser user) {
		...
		if (pkvisit != null) {
			DataBaseHelper.execute(
					"update INS_SZYB_VISIT set del_flag='1' where PK_VISIT=?",
					pkvisit);
			DataBaseHelper
					.execute(
	...
	*//**
	...
	public void deleteJsInfo(String param, IUser user) {
		...
		String pkInsst = "";
		// 1.通过pkPv查询ins_szyb_st中主键信息
		String stSql = "select pk_insst as pkInsst from ins_szyb_st  where pk_pv=? and (del_flag='0' or del_flag is null)";
		...
		// 3.分别依次更新ins_szyb_st,ins_gzyb_st_xnh,ins_gzyb_st_xnh_d;
		String sql = "update ins_szyb_st set del_flag='1' where pk_pv=?";
		...
		sql = "update INS_SZYB_VISIT set EU_STATUS_ST='0' where PK_PV=?";
		DataBaseHelper.update(sql, new Object[] { pkPv });
	}
...
/*
	...
	public void deleteVisitInfo(String param, IUser user) {
		...
		String pkVisit = "";
		// 1.通过pkPv查询INS_SZYB_VISIT中主键信息
		String vistSql = "select PK_VISIT as pkVisit from INS_SZYB_VISIT  where pk_pv=? and (del_flag='0' or del_flag is null) and EU_STATUS_ST='0'";
...
/*	*//** 门诊 *//*
	*//**
	...
	/*public void updatePvcodeIns(String param, IUser user) {
		String pkpv = JsonUtil.getFieldValue(param, "pkpv");
		String jydjh = JsonUtil.getFieldValue(param, "jydjh");
		if (pkpv != null && jydjh != null) {
			DataBaseHelper.execute(
					"update INS_SZYB_VISIT set PVCODE_INS=? where PK_PV=?",
					jydjh, pkpv);
		} else {
			throw new BusException("传入参数有空值");
		}
	...
	*//**
	...
	public void updateCodePc(String param, IUser user) {
		...
		if (pkpv != null && codePc != null) {
			DataBaseHelper
					.execute(
							"UPDATE INS_GZYB_VISIT_CITY SET CODE_PC=? where PK_VISIT IN (SELECT PK_VISIT FROM INS_SZYB_VISIT WHERE PK_PV=?)",
							codePc, pkpv);
		} else {
			throw new BusException("传入参数有空值");
		}
	...
	public void updateIDNOMZMB(String param, IUser user) {
		...
		if (pkpv != null && idnomzmb != null) {
			DataBaseHelper
					.execute(
							"UPDATE INS_SZYB_VISIT_CITY SET IDNO_MZMB=? where PK_VISIT IN (SELECT PK_VISIT FROM INS_GZYB_VISIT WHERE PK_PV=?)",
							idnomzmb, pkpv);
		} else {
			throw new BusException("传入参数有空值");
		}
	...
	public void updatePzh(String param, IUser user) {
		...
		if (pkpv != null && number != null) {
			if ("0".equals(type)) {
				DataBaseHelper
						.execute(
								"UPDATE INS_GZYB_VISIT_CITY SET IDNO_GS=? where PK_VISIT IN (SELECT PK_VISIT FROM INS_SZYB_VISIT WHERE PK_PV=?)",
								number, pkpv);
			}
			if ("1".equals(type)) {
				DataBaseHelper
						.execute(
								"UPDATE INS_GZYB_VISIT_CITY SET IDNO_SY=? where PK_VISIT IN (SELECT PK_VISIT FROM INS_SZYB_VISIT WHERE PK_PV=?)",
								number, pkpv);
			}
		} else {
			throw new BusException("传入参数有空值");

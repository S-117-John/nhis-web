package com.zebone.nhis.compay.ins.zsrm.vo;

import com.alibaba.fastjson.annotation.JSONField;

import java.math.BigDecimal;
import java.util.Date;

public class InsQgybSetInfo {
    /**
     * 就诊id
     */
    @JSONField(name = "mdtrt_id",ordinal = 0)
    private String mdtrtId;
    /**
     * 结算id
     */
    @JSONField(name = "setl_id",ordinal = 1)
    private String setlId;
    /**
     * 定点医疗机构名称
     */
    @JSONField(name = "fixmedins_name",ordinal = 2)
    private String fixmedinsName;
    /**
     * 定点医疗机构代码
     */
    @JSONField(name = "fixmedins_code",ordinal = 3)
    private String fixmedinsCode;
    /**
     * 医保结算等级
     */
    @JSONField(name = "hi_setl_lv",ordinal = 4)
    private String hiSetlLv;
    /**
     * 医保编号
     */
    @JSONField(name = "hi_no",ordinal = 5)
    private String hiNo;
    /**
     * 病案号
     */
    @JSONField(ordinal = 6)
    private String medcasno;
    /**
     * 申报时间
     */
    @JSONField(name = "dcla_time",format = "yyyy-MM-dd HH:mm:ss",ordinal = 7)
    private Date dclaTime;
    /**
     * 人员姓名
     */
    @JSONField(name = "psn_name",ordinal = 8)
    private String psnName;
    /**
     * 性别
     */
    @JSONField(ordinal = 9)
    private String gend;
    /**
     * 出生日期
     */
    @JSONField(ordinal = 10,format = "yyyy-MM-dd")
    private Date brdy;
    /**
     * 年龄(岁)
     */
    @JSONField(ordinal = 11)
    private int age;
    /**
     * 国籍
     */
    @JSONField(ordinal = 12)
    private String ntly;
    /**
     * 年龄(月)
     */
    @JSONField(name = "nwb_age",ordinal = 13)
    private int nwbAge;
    /**
     * 民族
     */
    @JSONField(ordinal = 14)
    private String naty;
    /**
     * 患者证件类别
     */
    @JSONField(name = "patn_cert_type",ordinal = 15)
    private String patnCertType;
    /**
     * 证件号码
     */
    @JSONField(ordinal = 16)
    private String certno;
    /**
     * 职业
     */
    @JSONField(ordinal = 17)
    private String prfs;
    /**
     * 现住址
     */
    @JSONField(name = "curr_addr",ordinal = 18)
    private String currAddr;
    /**
     * 单位名称
     */
    @JSONField(name = "emp_name",ordinal = 19)
    private String empName;
    /**
     * 单位地址
     */
    @JSONField(name = "emp_addr",ordinal = 20)
    private String empAddr;
    /**
     * 单位电话
     */
    @JSONField(name = "emp_tel",ordinal = 21)
    private String empTel;
    /**
     * 邮编
     */
    @JSONField(ordinal = 22)
    private String poscode;
    /**
     * 联系人姓名
     */
    @JSONField(name = "coner_name",ordinal = 23)
    private String conerName;
    /**
     * 与患者关系
     */
    @JSONField(name = "patn_rlts",ordinal = 24)
    private String patnRlts;
    /**
     * 联系人地址
     */
    @JSONField(name = "coner_addr",ordinal = 25)
    private String conerAddr;
    /**
     * 联系人电话
     */
    @JSONField(name = "coner_tel",ordinal = 26)
    private String conerTel;
    /**
     * 医保类型
     */
    @JSONField(name = "hi_type",ordinal = 27)
    private String hiType;
    /**
     * 参保地
     */
    @JSONField(ordinal = 28)
    private String insuplc;
    /**
     * 特殊人员类型
     */
    @JSONField(name = "sp_psn_type",ordinal = 29)
    private String spPsnType;
    /**
     * 新生儿入院类型
     */
    @JSONField(name = "nwb_adm_type",ordinal = 30)
    private String nwbAdmType;
    /**
     * 新生儿出生体重
     */
    @JSONField(name = "nwb_bir_wt",ordinal = 31)
    private BigDecimal nwbBirWt;
    /**
     * 新生儿入院体重
     */
    @JSONField(name = "nwb_adm_wt",ordinal = 32)
    private BigDecimal nwbAdmWt;
    /**
     * 门诊特慢病诊断科别
     */
    @JSONField(name = "opsp_diag_caty",ordinal = 33)
    private String opspDiagCaty;
    /**
     * 门诊特慢病就诊日期
     */
    @JSONField(name = "opsp_mdtrt_date",format = "yyyy-MM-dd HH:mm:ss",ordinal = 34)
    private Date  opspMdtrtDate;
    /**
     * 住院医疗类别
     */
    @JSONField(name = "ipt_med_type",ordinal = 35)
    private String iptMedType;
    /**
     * 入院途径
     */
    @JSONField(name = "adm_way",ordinal = 36)
    private String admWay;
    /**
     * 治疗类别
     */
    @JSONField(name = "trt_type",ordinal = 37)
    private String trtType;
    /**
     * 入院时间
     */
    @JSONField(name = "adm_time",format = "yyyy-MM-dd HH:mm:ss",ordinal = 38)
    private Date admTime;
    /**
     * 入院科别
     */
    @JSONField(name = "adm_caty",ordinal = 39)
    private String admCaty;
    /**
     * 转科科别
     */
    @JSONField(name = "refldept_dept",ordinal = 40)
    private String refldeptDept;
    /**
     * 出院时间
     */
    @JSONField(name = "dscg_time",format = "yyyy-MM-dd HH:mm:ss",ordinal = 41)
    private Date dscgTime;
    /**
     * 出院科别
     */
    @JSONField(name = "dscg_caty",ordinal = 42)
    private String dscgCaty;
    /**
     * 实际住院天数
     */
    @JSONField(name = "act_ipt_days",ordinal = 43)
    private int actIptDays;
    /**
     * 西医诊断
     */
    @JSONField(name = "otp_wm_dise",ordinal = 44)
    private String otpWmDise;
    /**
     * 西医诊断疾病代码
     */
    @JSONField(name = "wm_dise_code",ordinal = 45)
    private String wmDiseCode;
    /**
     * 中医诊断
     */
    @JSONField(name = "otp_tcm_dise",ordinal = 46)
    private String otpTcmDise;
    /**
     * 中医诊断代码
     */
    @JSONField(name = "tcm_dise_code",ordinal = 47)
    private String tcmDiseCode;
    /**
     * 诊断代码计数
     */
    @JSONField(name = "diag_code_cnt",ordinal = 48)
    private int diagCodeCnt;
    /**
     * 手术操作代码计数
     */
    @JSONField(name = "oprn_oprt_code_cnt",ordinal = 49)
    private int oprnOprtCodeCnt;
    /**
     * 呼吸机使用时长
     */
    @JSONField(name = "vent_used_dura",ordinal = 50)
    private String ventUsedDura;
    /**
     * 颅脑损伤患者入院前昏迷时长
     */
    @JSONField(name = "pwcry_bfadm_coma_dura",ordinal = 51)
    private String pwcryBfadmComaDura;
    /**
     * 颅脑损伤患者入院后昏迷时长
     */
    @JSONField(name = "pwcry_afadm_coma_dura",ordinal = 52)
    private String pwcryAfadmComaDura;
    /**
     * 输血品种
     */
    @JSONField(name = "bld_cat",ordinal = 53)
    private String bldCat;
    /**
     * 输血量
     */
    @JSONField(name = "bld_amt",ordinal = 54)
    private int bldAmt;
    /**
     * 输血计量单位
     */
    @JSONField(name = "bld_unt",ordinal = 55)
    private String bldUnt;
    /**
     * 特级护理天数
     */
    @JSONField(name = "spga_nurscare_days",ordinal = 56)
    private int spgaNurscareDays;
    /**
     * 一级护理天数
     */
    @JSONField(name = "lv1_nurscare_days",ordinal = 57)
    private int lv1NurscareDays;
    /**
     * 二级护理天数
     */
    @JSONField(name = "scd_nurscare_days",ordinal = 58)
    private int scdNurscareDays;
    /**
     * 三级护理天数
     */
    @JSONField(name = "lv3_nurscare_days",ordinal = 59)
    private int lv3NurscareDays;
    /**
     * 离院方式
     */
    @JSONField(name = "dscg_way",ordinal = 60)
    private String dscgWay;
    /**
     * 拟接收机构名称
     */
    @JSONField(name = "acp_medins_name",ordinal = 61)
    private String acpMedinsName;
    /**
     * 拟接收机构代码
     */
    @JSONField(name = "acp_optins_code",ordinal = 62)
    private String acpOptinsCode;
    /**
     * 票据代码
     */
    @JSONField(name = "bill_code",ordinal = 63)
    private String billCode;
    /**
     * 票据号码
     */
    @JSONField(name = "bill_no",ordinal = 64)
    private String billNo;
    /**
     * 业务流水号
     */
    @JSONField(name = "biz_sn",ordinal = 65)
    private String bizSn;
    /**
     * 出院31天内再住院计划标志
     */
    @JSONField(name = "days_rinp_flag_31",ordinal = 66)
    private String daysRinpFlag31;
    /**
     * 出院31天内再住院目的
     */
    @JSONField(name = "days_rinp_pup_31",ordinal = 67)
    private String daysRinpPup31;
    /**
     * 主诊医师姓名
     */
    @JSONField(name = "chfpdr_name",ordinal = 68)
    private String chfpdrName;
    /**
     * 主诊医师代码
     */
    @JSONField(name = "chfpdr_code",ordinal = 69)
    private String chfpdrCode;
    /**
     * 结算开始日期
     */
    @JSONField(name = "setl_begn_date",format = "yyyy-MM-dd HH:mm:ss",ordinal = 70)
    private Date setlBegnDate;
    /**
     * 结算结束日期
     */
    @JSONField(name = "setl_end_date",format = "yyyy-MM-dd HH:mm:ss",ordinal = 71)
    private Date setlEndDate;
    /**
     * 个人自付
     */
    @JSONField(name = "psn_selfpay",ordinal = 72)
    private BigDecimal psnSelfpay;
    /**
     * 个人自费
     */
    @JSONField(name = "psn_ownpay",ordinal = 73)
    private BigDecimal psnOwnpay;
    /**
     * 个人账户支出
     */
    @JSONField(name = "acct_pay",ordinal = 74)
    private BigDecimal acctPay;
    /**
     * 个人现金支付
     */
    @JSONField(name = "psn_cashpay",ordinal = 75)
    private BigDecimal psnCashpay;
    /**
     * 医保支付方式
     */
    @JSONField(name = "hi_paymtd",ordinal = 76)
    private String hiPaymtd;
    /**
     * 医保机构
     */
    @JSONField(name = "hsorg",ordinal = 77)
    private String hsorg;
    /**
     * 医保机构经办人
     */
    @JSONField(name = "hsorg_opter",ordinal = 78)
    private String hsorgOpter;
    /**
     * 医疗机构填报部门
     */
    @JSONField(name = "medins_fill_dept",ordinal = 79)
    private String medinsFillDept;
    /**
     * 医疗机构填报人
     */
    @JSONField(name = "medins_fill_psn",ordinal = 80)
    private String medinsFillPsn;

    public String getMdtrtId() {
        return mdtrtId;
    }

    public void setMdtrtId(String mdtrtId) {
        this.mdtrtId = mdtrtId;
    }

    public String getSetlId() {
        return setlId;
    }

    public void setSetlId(String setlId) {
        this.setlId = setlId;
    }

    public String getFixmedinsName() {
        return fixmedinsName;
    }

    public void setFixmedinsName(String fixmedinsName) {
        this.fixmedinsName = fixmedinsName;
    }

    public String getFixmedinsCode() {
        return fixmedinsCode;
    }

    public void setFixmedinsCode(String fixmedinsCode) {
        this.fixmedinsCode = fixmedinsCode;
    }

    public String getHiSetlLv() {
        return hiSetlLv;
    }

    public void setHiSetlLv(String hiSetlLv) {
        this.hiSetlLv = hiSetlLv;
    }

    public String getHiNo() {
        return hiNo;
    }

    public void setHiNo(String hiNo) {
        this.hiNo = hiNo;
    }

    public String getMedcasno() {
        return medcasno;
    }

    public void setMedcasno(String medcasno) {
        this.medcasno = medcasno;
    }

    public Date getDclaTime() {
        return dclaTime;
    }

    public void setDclaTime(Date dclaTime) {
        this.dclaTime = dclaTime;
    }

    public String getPsnName() {
        return psnName;
    }

    public void setPsnName(String psnName) {
        this.psnName = psnName;
    }

    public String getGend() {
        return gend;
    }

    public void setGend(String gend) {
        this.gend = gend;
    }

    public Date getBrdy() {
        return brdy;
    }

    public void setBrdy(Date brdy) {
        this.brdy = brdy;
    }

    public int getAge() {
        return age;
    }

    public void setAge(int age) {
        this.age = age;
    }

    public String getNtly() {
        return ntly;
    }

    public void setNtly(String ntly) {
        this.ntly = ntly;
    }

    public int getNwbAge() {
        return nwbAge;
    }

    public void setNwbAge(int nwbAge) {
        this.nwbAge = nwbAge;
    }

    public String getNaty() {
        return naty;
    }

    public void setNaty(String naty) {
        this.naty = naty;
    }

    public String getPatnCertType() {
        return patnCertType;
    }

    public void setPatnCertType(String patnCertType) {
        this.patnCertType = patnCertType;
    }

    public String getCertno() {
        return certno;
    }

    public void setCertno(String certno) {
        this.certno = certno;
    }

    public String getPrfs() {
        return prfs;
    }

    public void setPrfs(String prfs) {
        this.prfs = prfs;
    }

    public String getCurrAddr() {
        return currAddr;
    }

    public void setCurrAddr(String currAddr) {
        this.currAddr = currAddr;
    }

    public String getEmpName() {
        return empName;
    }

    public void setEmpName(String empName) {
        this.empName = empName;
    }

    public String getEmpAddr() {
        return empAddr;
    }

    public void setEmpAddr(String empAddr) {
        this.empAddr = empAddr;
    }

    public String getEmpTel() {
        return empTel;
    }

    public void setEmpTel(String empTel) {
        this.empTel = empTel;
    }

    public String getPoscode() {
        return poscode;
    }

    public void setPoscode(String poscode) {
        this.poscode = poscode;
    }

    public String getConerName() {
        return conerName;
    }

    public void setConerName(String conerName) {
        this.conerName = conerName;
    }

    public String getPatnRlts() {
        return patnRlts;
    }

    public void setPatnRlts(String patnRlts) {
        this.patnRlts = patnRlts;
    }

    public String getConerAddr() {
        return conerAddr;
    }

    public void setConerAddr(String conerAddr) {
        this.conerAddr = conerAddr;
    }

    public String getConerTel() {
        return conerTel;
    }

    public void setConerTel(String conerTel) {
        this.conerTel = conerTel;
    }

    public String getHiType() {
        return hiType;
    }

    public void setHiType(String hiType) {
        this.hiType = hiType;
    }

    public String getInsuplc() {
        return insuplc;
    }

    public void setInsuplc(String insuplc) {
        this.insuplc = insuplc;
    }

    public String getSpPsnType() {
        return spPsnType;
    }

    public void setSpPsnType(String spPsnType) {
        this.spPsnType = spPsnType;
    }

    public String getNwbAdmType() {
        return nwbAdmType;
    }

    public void setNwbAdmType(String nwbAdmType) {
        this.nwbAdmType = nwbAdmType;
    }

    public BigDecimal getNwbBirWt() {
        return nwbBirWt;
    }

    public void setNwbBirWt(BigDecimal nwbBirWt) {
        this.nwbBirWt = nwbBirWt;
    }

    public BigDecimal getNwbAdmWt() {
        return nwbAdmWt;
    }

    public void setNwbAdmWt(BigDecimal nwbAdmWt) {
        this.nwbAdmWt = nwbAdmWt;
    }

    public String getOpspDiagCaty() {
        return opspDiagCaty;
    }

    public void setOpspDiagCaty(String opspDiagCaty) {
        this.opspDiagCaty = opspDiagCaty;
    }

    public Date getOpspMdtrtDate() {
        return opspMdtrtDate;
    }

    public void setOpspMdtrtDate(Date opspMdtrtDate) {
        this.opspMdtrtDate = opspMdtrtDate;
    }

    public String getIptMedType() {
        return iptMedType;
    }

    public void setIptMedType(String iptMedType) {
        this.iptMedType = iptMedType;
    }

    public String getAdmWay() {
        return admWay;
    }

    public void setAdmWay(String admWay) {
        this.admWay = admWay;
    }

    public String getTrtType() {
        return trtType;
    }

    public void setTrtType(String trtType) {
        this.trtType = trtType;
    }

    public Date getAdmTime() {
        return admTime;
    }

    public void setAdmTime(Date admTime) {
        this.admTime = admTime;
    }

    public String getAdmCaty() {
        return admCaty;
    }

    public void setAdmCaty(String admCaty) {
        this.admCaty = admCaty;
    }

    public String getRefldeptDept() {
        return refldeptDept;
    }

    public void setRefldeptDept(String refldeptDept) {
        this.refldeptDept = refldeptDept;
    }

    public Date getDscgTime() {
        return dscgTime;
    }

    public void setDscgTime(Date dscgTime) {
        this.dscgTime = dscgTime;
    }

    public String getDscgCaty() {
        return dscgCaty;
    }

    public void setDscgCaty(String dscgCaty) {
        this.dscgCaty = dscgCaty;
    }

    public int getActIptDays() {
        return actIptDays;
    }

    public void setActIptDays(int actIptDays) {
        this.actIptDays = actIptDays;
    }

    public String getOtpWmDise() {
        return otpWmDise;
    }

    public void setOtpWmDise(String otpWmDise) {
        this.otpWmDise = otpWmDise;
    }

    public String getWmDiseCode() {
        return wmDiseCode;
    }

    public void setWmDiseCode(String wmDiseCode) {
        this.wmDiseCode = wmDiseCode;
    }

    public String getOtpTcmDise() {
        return otpTcmDise;
    }

    public void setOtpTcmDise(String otpTcmDise) {
        this.otpTcmDise = otpTcmDise;
    }

    public String getTcmDiseCode() {
        return tcmDiseCode;
    }

    public void setTcmDiseCode(String tcmDiseCode) {
        this.tcmDiseCode = tcmDiseCode;
    }

    public int getDiagCodeCnt() {
        return diagCodeCnt;
    }

    public void setDiagCodeCnt(int diagCodeCnt) {
        this.diagCodeCnt = diagCodeCnt;
    }

    public int getOprnOprtCodeCnt() {
        return oprnOprtCodeCnt;
    }

    public void setOprnOprtCodeCnt(int oprnOprtCodeCnt) {
        this.oprnOprtCodeCnt = oprnOprtCodeCnt;
    }

    public String getVentUsedDura() {
        return ventUsedDura;
    }

    public void setVentUsedDura(String ventUsedDura) {
        this.ventUsedDura = ventUsedDura;
    }

    public String getPwcryBfadmComaDura() {
        return pwcryBfadmComaDura;
    }

    public void setPwcryBfadmComaDura(String pwcryBfadmComaDura) {
        this.pwcryBfadmComaDura = pwcryBfadmComaDura;
    }

    public String getPwcryAfadmComaDura() {
        return pwcryAfadmComaDura;
    }

    public void setPwcryAfadmComaDura(String pwcryAfadmComaDura) {
        this.pwcryAfadmComaDura = pwcryAfadmComaDura;
    }

    public String getBldCat() {
        return bldCat;
    }

    public void setBldCat(String bldCat) {
        this.bldCat = bldCat;
    }

    public int getBldAmt() {
        return bldAmt;
    }

    public void setBldAmt(int bldAmt) {
        this.bldAmt = bldAmt;
    }

    public String getBldUnt() {
        return bldUnt;
    }

    public void setBldUnt(String bldUnt) {
        this.bldUnt = bldUnt;
    }

    public int getSpgaNurscareDays() {
        return spgaNurscareDays;
    }

    public void setSpgaNurscareDays(int spgaNurscareDays) {
        this.spgaNurscareDays = spgaNurscareDays;
    }

    public int getLv1NurscareDays() {
        return lv1NurscareDays;
    }

    public void setLv1NurscareDays(int lv1NurscareDays) {
        this.lv1NurscareDays = lv1NurscareDays;
    }

    public int getScdNurscareDays() {
        return scdNurscareDays;
    }

    public void setScdNurscareDays(int scdNurscareDays) {
        this.scdNurscareDays = scdNurscareDays;
    }

    public int getLv3NurscareDays() {
        return lv3NurscareDays;
    }

    public void setLv3NurscareDays(int lv3NurscareDays) {
        this.lv3NurscareDays = lv3NurscareDays;
    }

    public String getDscgWay() {
        return dscgWay;
    }

    public void setDscgWay(String dscgWay) {
        this.dscgWay = dscgWay;
    }

    public String getAcpMedinsName() {
        return acpMedinsName;
    }

    public void setAcpMedinsName(String acpMedinsName) {
        this.acpMedinsName = acpMedinsName;
    }

    public String getAcpOptinsCode() {
        return acpOptinsCode;
    }

    public void setAcpOptinsCode(String acpOptinsCode) {
        this.acpOptinsCode = acpOptinsCode;
    }

    public String getBillCode() {
        return billCode;
    }

    public void setBillCode(String billCode) {
        this.billCode = billCode;
    }

    public String getBillNo() {
        return billNo;
    }

    public void setBillNo(String billNo) {
        this.billNo = billNo;
    }

    public String getBizSn() {
        return bizSn;
    }

    public void setBizSn(String bizSn) {
        this.bizSn = bizSn;
    }

    public String getDaysRinpFlag31() {
        return daysRinpFlag31;
    }

    public void setDaysRinpFlag31(String daysRinpFlag31) {
        this.daysRinpFlag31 = daysRinpFlag31;
    }

    public String getDaysRinpPup31() {
        return daysRinpPup31;
    }

    public void setDaysRinpPup31(String daysRinpPup31) {
        this.daysRinpPup31 = daysRinpPup31;
    }

    public String getChfpdrName() {
        return chfpdrName;
    }

    public void setChfpdrName(String chfpdrName) {
        this.chfpdrName = chfpdrName;
    }

    public String getChfpdrCode() {
        return chfpdrCode;
    }

    public void setChfpdrCode(String chfpdrCode) {
        this.chfpdrCode = chfpdrCode;
    }

    public Date getSetlBegnDate() {
        return setlBegnDate;
    }

    public void setSetlBegnDate(Date setlBegnDate) {
        this.setlBegnDate = setlBegnDate;
    }

    public Date getSetlEndDate() {
        return setlEndDate;
    }

    public void setSetlEndDate(Date setlEndDate) {
        this.setlEndDate = setlEndDate;
    }

    public BigDecimal getPsnSelfpay() {
        return psnSelfpay;
    }

    public void setPsnSelfpay(BigDecimal psnSelfpay) {
        this.psnSelfpay = psnSelfpay;
    }

    public BigDecimal getPsnOwnpay() {
        return psnOwnpay;
    }

    public void setPsnOwnpay(BigDecimal psnOwnpay) {
        this.psnOwnpay = psnOwnpay;
    }

    public BigDecimal getAcctPay() {
        return acctPay;
    }

    public void setAcctPay(BigDecimal acctPay) {
        this.acctPay = acctPay;
    }

    public BigDecimal getPsnCashpay() {
        return psnCashpay;
    }

    public void setPsnCashpay(BigDecimal psnCashpay) {
        this.psnCashpay = psnCashpay;
    }

    public String getHiPaymtd() {
        return hiPaymtd;
    }

    public void setHiPaymtd(String hiPaymtd) {
        this.hiPaymtd = hiPaymtd;
    }

    public String getHsorg() {
        return hsorg;
    }

    public void setHsorg(String hsorg) {
        this.hsorg = hsorg;
    }

    public String getHsorgOpter() {
        return hsorgOpter;
    }

    public void setHsorgOpter(String hsorgOpter) {
        this.hsorgOpter = hsorgOpter;
    }

    public String getMedinsFillDept() {
        return medinsFillDept;
    }

    public void setMedinsFillDept(String medinsFillDept) {
        this.medinsFillDept = medinsFillDept;
    }

    public String getMedinsFillPsn() {
        return medinsFillPsn;
    }

    public void setMedinsFillPsn(String medinsFillPsn) {
        this.medinsFillPsn = medinsFillPsn;
    }
}

package com.zebone.nhis.compay.ins.shenzhen.service.szxnh;
...
import com.zebone.nhis.common.module.compay.ins.shenzhen.xnh.InsXnhRefdoc;
...
import com.zebone.nhis.common.module.compay.ins.shenzhen.xnh.InsSzybStXnhDt;
import com.zebone.nhis.common.support.CommonUtils;
import com.zebone.nhis.common.support.DateUtils;
import com.zebone.nhis.compay.ins.shenzhen.dao.szxnh.SzxnhMapper;
...
import com.zebone.nhis.compay.ins.shenzhen.vo.szxnh.StVo;
 ...
 */
@Service
public class SzybXnhService {

	@Resource
	private SzxnhMapper szxnhMapper;

	/**
	 * 015001012001 入院登记：2.保存转诊单
	 * 
	 * @param param
	 *            {"insXnhRefdoc":"转诊单记录"}
	 * @param user
	...
	/**
	 * 	015001012002 入院登记
	 * 3. 关联转诊单与患者就诊-就诊记录pv_encounter的pk_pv更新转诊单就诊主键pk_pv
	 * 
	 * @param param
	 *            {"pkPi":"患者主键","pkPv":"就诊主键"}
	 * @param user
	...
	public void deleteVisitByPkPv(String param, IUser user) {
		...
		if (pkPv == null || "".equals(pkPv))
			throw new BusException("未获得就诊信息！");
		String pkVisit = "";
		String sql = "SELECT PK_VISIT FROM INS_SZYB_VISIT WHERE PK_PV=? AND DEL_FLAG='0'";
		...
		String visitSql = "UPDATE INS_SZYB_VISIT SET DEL_FLAG='1' WHERE pk_pv=?";
	...
	public void saveXnhStDt(String param, IUser user) {
		User u = (User) user;
		List<InsSzybStXnhDt> xnhStDts = JsonUtil.readValue(param,
				new TypeReference<List<InsSzybStXnhDt>>() {
				});
		if (xnhStDts != null && xnhStDts.size() > 0) {
			for (InsSzybStXnhDt xnhStDt : xnhStDts) {
	...
	public void deleteXnJsInfo(String param, IUser user) {
		...
		String stSql = "select pk_insst  from ins_szyb_st  where pk_pv=? and (del_flag='0' or del_flag is null)";
	...
	/**
	 * 015001012010 根据pkPv查询新农合登记信息和转诊单信息
	 * 
	 * @param param
	 *            {"pkPv":"就诊主键"}
	 * @param user
	...
	public Map<String, Object> qryXnhPiInfoByPkPv(String param, IUser user) {
		...
		return szxnhMapper.qryXnhPiInfoByPkPv(pkPv,type);
	}

	/**
	 * 015001012011 根据患者住院登记流水号查询患者结算信息
	 * 
	 * @param param
	 * @param user
	 * @return
	...
	public Map<String, Object> qryXnhJsInfo(String param, IUser user) {
		...
		return szxnhMapper.qryXnhJsInfo(pkVisit);

	}

	/**
	 * 015001012012通过患者主键跟就诊主键获取已上传未结算的收费明细表-患者自费总额
	 * 
	 * @param param
	 * @param user
	 * @return
	...
	public String queryShareAmount(String param, IUser user) {
		...
		if (pkPv == null || "".equals(pkPv))
			throw new BusException("就诊信息为空！");
		String amtPi = szxnhMapper.queryShareAmount(paramMap);
	...
	public void saveXnhStInfo(String param, IUser user) {
		...
		String sql = "delete from ins_szyb_st where pk_visit=?";
	...
	/**
	 * 015001012014根据就诊主键pk_pv查询住院收费明细bl_ip_dt的医保上传标志flag_insu为0的记录
	 * 
	 * @param param
	 * @param user
	 * @return
	...
	public List<XnhBlipdtVo> qryBdItemAndOrderByPkPv(String param, IUser user) {
		...
		List<XnhBlipdtVo> list = szxnhMapper.qryBdItemAndOrderByPkPv(pkPv);
		return list;
	}

	/**
	 * 015001012015根据pkPv查询患者出入院状态及关联平台科室
	 * 
	 * @param param
	 * @param user
	 * @return
	...
	public Map<String, Object> qryPreInpTempInfo(String param, IUser user) {
		...
		return szxnhMapper.qryPreInpTempInfo(pkPv);

	}

	/**
	 * 015001012016 获取结算单打印数据（基本数据和明细数据）
	 * 
	 * @param param
	 * @param user
	 * @return
	...
	public Map<String, Object> qrySettlePageData(String param, IUser user) {
		...
		// 2.查询结算基本信息和明细信息
		SettlePageHeaderVo pageVo = szxnhMapper.qryPageHeaderVo(pkPv);
		List<SettlePageDtVo> dtVos = szxnhMapper.qryPageDtVos(pkPv);
		// 3.组合基本信息
		if (pageVo != null) {
			// 3.1年龄
	...
	public String cancelInpRegister(String param, IUser user) {
		...
		String inpNo = szxnhMapper.qryXnhCodePv(pkPv);
		return inpNo;
	}

	/**
	 * 015001012017获取科室对照信息
	 * 
	 * @param param
	 * @param user
	 * @return
	...
	public XnhDeptVo qryXNhDeptVo(String param, IUser user) {
		...
		XnhDeptVo xnhDeptVo = szxnhMapper.qryXNhDeptVo(pkeptadmit);
		return xnhDeptVo;
	}

	/**
	 * 015001012018通过pkpv获取诊断信息
	 * 
	 * @param param
	 * @param user
	 * @return
	...
	public DiagVo qryDiagVo(String param, IUser user) {
		...
		DiagVo diagVo = szxnhMapper.qryDiagVo(pkpv);
		return diagVo;
	}

	/**
	 * 015001012019通过pkpv更新门诊或住院的flag_insu标记
	 * 
	 * @param param
	 * @param user
	 */
	...
	/**
	 * 015001012008
	 * 通过医保结算登记主键PK_INSST将PkSettle结算主键更新到ins_szyb_st表同时更新INS_SZYB_VISIT表
	 * 
	 * @param param
	 * @param user
	 */
	public void updatePkSettle(String param, IUser user) {
		...
		if (pkpv != null && pkSettle != null) {
			if ("1".equals(pkSettle)) {
				DataBaseHelper
						.execute(
								"update ins_szyb_st set PK_SETTLE='' where PK_PV=? and del_flag='0'",
								pkpv);
				DataBaseHelper
						.execute(
								"update INS_SZYB_VISIT set EU_STATUS_ST='0' where PK_PV=? and del_flag='0'",
								pkpv);
			} else {
				DataBaseHelper
						.execute(
				...
				DataBaseHelper
						.execute(
								"update INS_SZYB_VISIT set EU_STATUS_ST='1' where PK_PV=? and del_flag='0'",
								pkpv);
				DataBaseHelper
						.execute(

...
import com.zebone.nhis.ma.pub.sd.vo.EnotePrintReqVo;
import com.zebone.nhis.ma.pub.sd.vo.EnotePrintResVo;
...
import com.zebone.nhis.ma.pub.sd.vo.OtherInfoVo;
...
import com.zebone.nhis.pro.zsba.mz.pub.dao.ZsbaEnoteMapper;
...
@Service
public class ZsbaEnoteService implements InitializingBean {
	...
	private String billCodePaper;	/**纸质票据种类代码*/
	private String perConfig;	//个性化配置
	private String hospCode;	//机构编码
	private String insuAddr;	//医保归属地
	private String resultStr = "S0000";//返回值状态 S0000：成功    其他：失败

	private String placeCode;	//深大门诊工号前面加M
	
	@Autowired private ZsbaEnoteMapper enoteDao;
	@Autowired private BoaiInvSettltService invSettltService;
...
//		paramMap.put("pkPv", "6478bafa6d7747d28247fcab10ece1ba");
		...
		if(inform!=null && inform.size()>0){
			...
			resVo.setCardNo(StringUtils.isNotEmpty(CommonUtils.getString(inform.get("cardNo")))?CommonUtils.getString(inform.get("cardNo")):"******");
		}
...
//			resVo.setPatientNo(CommonUtils.getString(pvMap.get("patientNo")));
			resVo.setPatientId(CommonUtils.getString(pvMap.get("codeOp")));
			resVo.setPatientNo(CommonUtils.getString(pvMap.get("codeOp")));
...
//			resVo.setPatientNo(CommonUtils.getString(pvMap.get("patientNo")));
		...
		//医保报销部分也要算入交费渠道列表里(应该是perConfig=pskq才会走下面这两个判断逻辑)
		if(resVo.getAccountPay()!=0D){
			PayChannelDetailVo dtl = new PayChannelDetailVo();
			dtl.setPayChannelCode("YBK");
			dtl.setPayChannelValue(resVo.getAccountPay());
...
//			resVo.setSelfCashPay(MathUtils.add(resVo.getSelfCashPay(), amtCe));
		 ...
		 * 删除金额为0的记录
		 */
		List<PayChannelDetailVo>  removeList = new ArrayList<PayChannelDetailVo>();
		for(PayChannelDetailVo pay : resVo.getPayChannelDetail()){
			if(BigDecimal.ZERO.compareTo(new BigDecimal(pay.getPayChannelValue().toString()))==0){
				removeList.add(pay);
			}
		}
		resVo.getPayChannelDetail().removeAll(removeList);
	...
	private EnoteResDataVo invEBillOutpatient(Object[] args){
		...
		resVo.setAlipayCode(null);
		//根据结算主键获取本次缴费微信支付订单号
		Map<String,Object> wechatTrade = enoteDao.qryWechatTradeNo(paramMap);
		if(wechatTrade!=null){
			String tradeNo = CommonUtils.getString(wechatTrade.get("tradeNo"));
			if(StringUtils.isNotEmpty(tradeNo) && tradeNo.startsWith("42")) {
				resVo.setWeChatOrderNo(CommonUtils.getString(wechatTrade.get("tradeNo")));
			}else {
				resVo.setWeChatOrderNo(null);
			}
		}else{
		...
		Map<String,Object> inform = enoteDao.qryInformInfo(paramMap);
		if(inform!=null && inform.size()>0){
			String mobile = CommonUtils.getString(inform.get("tel"));
			if(StringUtils.isNotEmpty(mobile) && isNum(mobile) && mobile.length()==11){
				resVo.setTel(mobile);
			}
			resVo.setEmail(CommonUtils.getString(inform.get("email")));
			resVo.setPayerType(CommonUtils.getString(inform.get("payerType")));
...
//			resVo.setPatientNo(CommonUtils.getString(pvMap.get("patientNo")));
			...
			resVo.setPatientNo(CommonUtils.getString(pvMap.get("codeOp")));
			resVo.setConsultationDate(CommonUtils.getString(pvMap.get("inHospitalDate")));
			String patientSex = CommonUtils.getString(pvMap.get("sex"));
			if(StringUtils.isEmpty(patientSex) || patientSex.equals("未说明的性别")){
				patientSex = "未知";//性别为空或者[未说明的性别]则默认为未知
			}
			resVo.setSex(patientSex);
			resVo.setAge(CommonUtils.getString(pvMap.get("age")));
			resVo.setHospitalArea(CommonUtils.getString(pvMap.get("hospitalArea")));
			resVo.setBedNo(CommonUtils.getString(pvMap.get("bedNo")));
			resVo.setCaseNumber(CommonUtils.getString(pvMap.get("caseNumber")));
		...
		resVo.setSelfConceitedAmt(0D);
		/**
		 * 20210507根据结算主键查询医保结算记录计算个人自付金额
		 */
		InsQgybSt insQgybSt = DataBaseHelper.queryForBean("select * from INS_QGYB_ST where PK_SETTLE=?", InsQgybSt.class, new Object[]{String.valueOf(paramMap.get("pkSettle"))});
		Double amtZifu = 0d;
		if(insQgybSt!=null){
			/**全国医保**/
			//查询医保结算费用明细列表(自付=费用明细内符合政策范围总金额-基金支付总额)
			Map<String, Object> qryMap = new HashMap<String, Object>();
			qryMap.put("pkSettle", String.valueOf(paramMap.get("pkSettle")));
			List<Map<String, Object>> settleCgList = opCgQueryMapper.queryQgybSettleDetail(qryMap);
			BigDecimal sumInscpScpAmt = BigDecimal.ZERO;
			for(Map<String, Object> cg : settleCgList) {
				sumInscpScpAmt = sumInscpScpAmt.add(new BigDecimal(String.valueOf(cg.get("inscpScpAmt"))));//费用明细内符合政策范围总金额
			}
			amtZifu = sumInscpScpAmt.setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue()-insQgybSt.getFundPaySumamt();
			//amtZifu = insQgybSt.getMedfeeSumamt()-insQgybSt.getFundPaySumamt()-insQgybSt.getFulamtOwnpayAmt()-insQgybSt.getOverlmtSelfpay()-insQgybSt.getPreselfpayAmt()-(insQgybSt.getHospPartAmt()!=null?insQgybSt.getHospPartAmt():0d);
		}else{
			/**省工伤医保**/
			InsSgsybSt insSgsybSt = DataBaseHelper.queryForBean("select * from INS_SGSYB_ST where PK_SETTLE=?", InsSgsybSt.class, new Object[]{String.valueOf(paramMap.get("pkSettle"))});
			if(insSgsybSt!=null){
				amtZifu = Double.parseDouble(insSgsybSt.getBka831());
			}
		}
		resVo.setSelfPayAmt((new BigDecimal(amtZifu)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue());//个人自付
		resVo.setOwnPay((new BigDecimal(amtPi-amtZifu)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue());//个人自费
		resVo.setSelfCashPay(amtPi);//个人现金支付(存在个账支付的话下面会再减掉)
		resVo.setCashPay(0D);
		resVo.setChequePay(0D);
		resVo.setTransferAccountPay(0D);
		...
		// 其他扩展信息
		List<OtherInfoVo> oiList = new ArrayList<OtherInfoVo>();
		OtherInfoVo oiv = new OtherInfoVo();
		oiv.setInfoNo(1);
		oiv.setInfoName("院内优惠/政策性减免");
		oiv.setInfoValue(invInfo.get("amountDisc").toString());
		oiList.add(oiv);
		resVo.setOtherInfo(oiList);
		
		resVo.setOtherMedicalList(null);	
		resVo.setPayChannelDetail(enoteDao.qryStDepoList(paramMap));//查询结算首付款信息(缴款渠道需要电子票据平台对照)
		
		if(paramMap.containsKey("pkSettle") && !CommonUtils.isEmptyString(CommonUtils.getString(paramMap.get("pkSettle")))){
			//个人账户支付
			Double accountPay = enoteDao.qryAumByPkSettleZsrm(paramMap);
			if(accountPay != null){
				resVo.setAccountPay(accountPay);
				//个人现金=个人自费-个帐
				BigDecimal selCashPay = new BigDecimal(amtPi).subtract(BigDecimal.valueOf(accountPay)).setScale(2, BigDecimal.ROUND_HALF_UP);
				resVo.setSelfCashPay(selCashPay.doubleValue());
			}else{
				resVo.setAccountPay(0D);
			}
			if(invList!=null && invList.size()>0){
		...
		//校验总金额和缴费渠道金额是否一致，修正缴款渠道对照码
		Double amtChk = 0D;
		for(PayChannelDetailVo pay : resVo.getPayChannelDetail()){
			pay.setPayChannelCode(zsbaPayChannelCodeTran(pay.getPayChannelCode()));
			amtChk = MathUtils.add(amtChk, pay.getPayChannelValue());
		}
		if(amtChk.compareTo(resVo.getTotalAmt())!=0){
			Double amtCe = MathUtils.sub(resVo.getTotalAmt(), amtChk);
			...
			*/
			PayChannelDetailVo dtl = new PayChannelDetailVo();
			dtl.setPayChannelCode("02");//博爱现金支付
			dtl.setPayChannelValue(amtCe);
			if(resVo.getPayChannelDetail()==null){
				List<PayChannelDetailVo> dtlList = new ArrayList<>();
				resVo.setPayChannelDetail(dtlList);
	...
	private EnoteResDataVo writeOffEbill(Object[] args,List<Map<String,Object>> ebillListMap){
		...
		Map<String,Object> reqMap = new HashMap<>(16);
		reqMap.put("billBatchCode", ebillListMap.get(0).get("ebillbatchcode"));//电子票据代码
		reqMap.put("billNo", ebillListMap.get(0).get("billno"));//电子票据号码
		reqMap.put("reason", ebillListMap.get(0).get("reason"));//冲红原因
		reqMap.put("operator", u.getNameEmp());//经办人
		reqMap.put("busDateTime", DateUtils.getDate("yyyyMMddHHmmssSSS"));//业务发生时间
		reqMap.put("placeCode", getPlaceCode(MapUtils.getString(ebillListMap.get(0),"euType"),u));//开票点编码
		String dataJson = JsonUtil.writeValueAsString(reqMap);//将Data内容转换为json格式再base64编码,编码字符集UTF-8
		EnoteResDataVo dataVo = enoteRts(dataJson,"1.0","writeOffEBill");//发送请求
...
//			List<BlInvoice> invo = (List<BlInvoice>) invMap.get("inv");
	...
	public Map<String,Object> billCancel(Object[] args){
		...
		if(resVo!=null && !CommonUtils.isEmptyString(resVo.getResult())){
			...
			billLog.info("电子票据平台冲红：{}", (dataVo!=null?dataVo.toString():"电子票据冲红结果为空"));
...
//			} else{
  ...
  public void zsrmRetSelefPayAmt(Map<String,Object> paramMap,EnotePrintReqVo resVo){
    ...
    * 
    * NHIS缴款方式：
    * 1现金、2支票、3银行卡、4患者账户、5内部转账、6单位记账、7微信、8支付宝、12一代社保卡、13三代电子凭证、14三代社保卡、91内部转账、92财务记账、99其他
    * 
    * 电子票据平台缴款渠道对照码：
    * 00其它、01POS刷卡、02现金、03转账、04支付宝、05微信、06支票、07卡支付、09自助机缴费、10软POS、11医保支付、12预交金、13公众平台
    */
   public String zsbaPayChannelCodeTran(String resVo){
	   if(StringUtils.isBlank(resVo)){
		   return "00";//默认00其他
	   }
	   switch (resVo){

package com.zebone.nhis.pro.zsba.compay.pub.support;
...
@Service
public class ZsbaInvSettltService {
	...
	public  Map<String, Object> invoData(String pkPv,String flagSpec,
			...
			String dtDeptType, BigDecimal InsuAmt, BigDecimal amtDisc) {
		...
		if(EnumerateParameter.ONE.equals(splitWay)){
			return handleAmt(invoInfos,user,pkPv,flagSpec,dateBegin,dateEnd,pkSettle,flagHbPrint, dtDeptType, InsuAmt, amtDisc);
	...
	private Map<String, Object> handleDept(List<InvInfoVo> invoInfos, User user,String dateBegin,String dateEnd,String pkPv) {
		...
		List<ZsbaBlInvoice> invoices = new ArrayList<ZsbaBlInvoice>();
		...
		for(int i = 1;i<=data.size();i++){
			...
			ZsbaBlInvoice inv = new ZsbaBlInvoice();
	 ...
	 * @return
	 */
	private Map<String, Object> handleAmt(List<InvInfoVo> invoInfos, User user, String pkPv, String flagSpec , 
			String dateBegin, String dateEnd,String pkSettle,String flagHbPrint, String dtDeptType, BigDecimal insuAmt, BigDecimal amtDisc) {
		...
		List<ZsbaBlInvoice> invs = new ArrayList<ZsbaBlInvoice>();
		...
		if(amtRe !=null ){
			 amt  = amtRe.getAmtSum();
			 //有医保支付的时候，需要减去医保支付才是真的自付金额
			  amtPi  = amtRe.getAmtPi()-insuAmt.doubleValue();
		}
		...
		//科室类型要用前端传过来的当前科室类型，而不是当前登录人的科室类型
/*		Map<String, Object> paraMap = new HashMap<String, Object>();
		...
		*/
		...
		for(InvInfoVo vo : invoInfos){
			...
			//这里的优惠金额是指结算页面手动输入的，所以无法用sql计算，只能在这里减去
			amt_pi  = amt_pi-amtDisc.doubleValue();

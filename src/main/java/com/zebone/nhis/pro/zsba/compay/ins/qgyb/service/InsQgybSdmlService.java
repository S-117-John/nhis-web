 ...
 */
@Service
public class InsQgybSdmlService {
	...
	public String saveSdml(String param,IUser user){
		
		String sql = "select top 2000 * from lsb_qgyb_yp where status is null";
		List<Map<String, Object>> list = DataBaseHelper.queryForList(sql);
		
		int dqNum = 0; //全部
		int addNum = 0; //新增的
		int upNum = 0; //修改国家码的
		int bgNum = 0; //数据库已存在无需改动
		int zfkNul = 0;//支付库不存在
		String msg = "";
		for (Map<String, Object> map : list) {
			sql = "select * from lsb_zfk where xmdm = ? ";
			Map<String, Object> zfkMap = DataBaseHelper.queryForMap(sql, map.get("sdm"));
			if(zfkMap!=null){
				//判断his有么有，没有的新增
				sql = "select * from ins_item where del_flag = '0' and xmbh = ? and yjjypbm = ? and yyclsyjzch = ? ";
				if(map.get("sjgg")==null){
					sql += " and (ypgg is null or replace(ypgg,' ','') = '') ";
				}else{
					sql += " and ypgg = '" + map.get("sjgg").toString()+"' ";
				}
				if(map.get("sjjx")==null){
					sql += " and (ypjx is null or replace(ypjx,' ','') = '') ";
				}else{
					sql += " and ypjx = '" + map.get("sjjx").toString()+"' ";
				}
				if(map.get("bzcz")==null){
					sql += " and (bzcz is null or replace(bzcz,' ','') = '') ";
				}else{
					sql += " and bzcz = '" + map.get("bzcz").toString()+"' ";
				}
				if(map.get("zxbzsl")==null){
					sql += " and (zxbzsl is null or replace(zxbzsl,' ','') = '') ";
				}else{
					sql += " and zxbzsl = '" + map.get("zxbzsl").toString()+"' ";
				}
				if(map.get("zxbzdw")==null){
					sql += " and (zxbzdw is null or replace(zxbzdw,' ','') = '') ";
				}else{
					sql += " and zxbzdw = '" + map.get("zxbzdw").toString()+"' ";
				}
				if(map.get("zxzjdw")==null){
					sql += " and (zxzjdw is null or replace(zxzjdw,' ','') = '') ";
				}else{
					sql += " and zxzjdw = '" + map.get("zxzjdw").toString()+"' ";
				}
				InsZsBaYbItem item = DataBaseHelper.queryForBean(sql, InsZsBaYbItem.class, map.get("sdm"), map.get("ypbwm"), map.get("pzwh"));
				if(item==null){
					item = new InsZsBaYbItem();
					//item.setBgsj(bgsj); //变更时间
					item.setPkHp("fcace0e12aba439991c0eb001eef02f3"); //医保类别
					item.setXmbh(map.get("sdm").toString());//项目编号
					item.setXmlb(zfkMap.get("xmlb").toString());//项目类别 1 药品，2 材料，3 诊疗
					item.setZwmc(map.get("zcmc").toString());//中文名称
					//item.setYwmc("");//英文名称
					item.setSpcode(hzToSzmdx(map.get("zcmc").toString()));//拼音码
					item.setdCode(getWBCode(map.get("zcmc").toString()));//其他码
					item.setFldm(zfkMap.get("y18xfldm")==null?"":zfkMap.get("y18xfldm").toString());//分类代码
					item.setYpgg(map.get("zcgg")==null?"":map.get("zcgg").toString());//规格
					item.setYpjx(map.get("zcjx")==null?"":map.get("zcjx").toString());//剂型
					//item.setZjm("");//助记码
					item.setYwmldj(zfkMap.get("ywmldj").toString());//药品目录登记 1甲类2乙类3自费
					item.setJbzfbl(new BigDecimal(zfkMap.get("jbylzfbl").toString()));//基本医疗自费比例
					item.setSqzfbl(new BigDecimal(zfkMap.get("sqmzzfbl").toString()));//社区自费比例
					item.setLxzfbl(new BigDecimal(zfkMap.get("lxzfbl").toString()));//离休自费比例
					//item.setGszfbl();//工伤自费比例
					item.setSyzfbl(new BigDecimal(zfkMap.get("syzfbl").toString()));//生育自费比例
					//item.setTmzfbl();//特定门诊自费比例
					//item.setBczfbl();//补充医疗自费比例
					item.setJzjg(map.get("ypmldj")==null?null:zfkMap.get("ypmldj").toString()==""?null:new BigDecimal(zfkMap.get("ypmldj").toString()));//基准价格
					item.setXjje(map.get("xjje")==null?null:zfkMap.get("xjje").toString()==""?null:new BigDecimal(zfkMap.get("xjje").toString()));//限价金额
					item.setLxxjje(map.get("lxxjje")==null?null:zfkMap.get("lxxjje").toString()==""?null:new BigDecimal(zfkMap.get("lxxjje").toString()));//离休限价金额
					//item.setJzkbz();//基准库标志
					//item.setXzkbz();//限制库标志
					//item.setTdkbz();//特定库标志
					//item.setCjlbz();//产检库标志
					//item.setBzdm();//病种代码
					item.setYjjypbm(map.get("ypbwm").toString());//药监局药品编码
					item.setYyclzczmc(map.get("zcmc").toString());//医用材料的注册证产品名称
					item.setYyclsyjzch(map.get("pzwh").toString());//医用材料的食药监注册号
					//item.setBz();//注册备注
					item.setScdw(map.get("ypqy").toString());//生产单位
					//item.setTxbz();//特项标志
					//item.setCwlr();//除外内容
					//item.setSm();//说明
					//item.setXmnh();//项目内涵
					//item.setKssj();//开始时间
					//item.setZssj(zssj);//终止时间
					item.setNationCode(map.get("ypdm").toString());
					item.setBzcz(map.get("bzcz")==null?null:map.get("bzcz").toString());
					item.setZxbzsl(map.get("zxbzsl")==null?null:map.get("zxbzsl").toString());
					item.setZxbzdw(map.get("zxbzdw")==null?null:map.get("zxbzdw").toString());
					item.setZxzjdw(map.get("zxzjdw")==null?null:map.get("zxzjdw").toString());
					DataBaseHelper.insertBean(item);
					addNum ++;
					System.out.println((addNum+upNum)+"");
				}else{
					if(!item.getNationCode().equals(map.get("ypdm").toString())){
						msg += item.getNationCode()+"|"+map.get("ypdm")+"|"+map.get("sdm")+"|"+map.get("pzwh")+"|"+map.get("sjgg")+"|"
								+map.get("sjjx");
						item.setModityTime(new Date());
						//item.setBgsj(new Date()); //变更时间
						item.setNationCode(map.get("ypdm").toString());
						DataBaseHelper.updateBeanByPk(item);
						upNum ++;
					}else{
						bgNum ++;
					}
			...
			sql = "update lsb_qgyb_yp set status = '1' where sdm = ? and ypbwm = ? and pzwh = ? and ypdm = ?  ";
			if(map.get("sjgg")==null){
				sql += " and sjgg is null";
			}else{
				sql += " and sjgg = '" + map.get("sjgg").toString()+"' ";
			}
			if(map.get("sjjx")==null){
				sql += " and sjjx is null";
			}else{
				sql += " and sjjx = '" + map.get("sjjx").toString()+"' ";
			}
			if(map.get("bzcz")==null){
				sql += " and bzcz is null";
			}else{
				sql += " and bzcz = '" + map.get("bzcz").toString()+"' ";
			}
			if(map.get("zxbzsl")==null){
				sql += " and zxbzsl is null";
			}else{
				sql += " and zxbzsl = '" + map.get("zxbzsl").toString()+"' ";
			}
			if(map.get("zxbzdw")==null){
				sql += " and zxbzdw is null";
			}else{
				sql += " and zxbzdw = '" + map.get("zxbzdw").toString()+"' ";
			}
			if(map.get("zxzjdw")==null){
				sql += " and zxzjdw is null";
			}else{
				sql += " and zxzjdw = '" + map.get("zxzjdw").toString()+"' ";
			}
			DataBaseHelper.execute(sql,
					 map.get("sdm"), map.get("ypbwm"), map.get("pzwh"), map.get("ypdm"));
			dqNum ++;
			System.out.println(msg);
			System.out.println("检查条数："+dqNum+",导入条数："+addNum+",修改条数："+upNum);
		}
		System.out.println("总数量："+list.size() + ",导入："+addNum+"条,修改："+upNum+"条,已存在无需改动："+bgNum+"条,支付库不存在："+zfkNul+"条,成功数量："+(addNum+upNum+bgNum+zfkNul)+"!");
		return "总数量："+list.size() + ",导入："+addNum+"条,修改："+upNum+"条,已存在无需改动："+bgNum+"条,支付库不存在："+zfkNul+"条,成功数量："+(addNum+upNum+bgNum+zfkNul)+"!";
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public String saveSdmlCj(String param,IUser user){
		String sql = "select * from lsb_qgyb_cjhz ";
		List<Map<String, Object>> list = DataBaseHelper.queryForList(sql);
		int cgNum = 0;
		int yyNum = 0;
		for (Map<String, Object> cjMap : list) {
			sql = "select * from lsb_qgyb_yp where zcmc = ?";
			Map<String, Object> map = DataBaseHelper.queryForMap(sql, cjMap.get("cpmc"));
			if(map!=null && map.get("sdm2")!=null){
				sql = "select * from lsb_zfk where xmdm = ? ";
				Map<String, Object> zfkMap = DataBaseHelper.queryForMap(sql, map.get("sdm2"));
				if(zfkMap!=null){
					sql = "select * from ins_item where yjjypbm= ?";
					Map<String, Object> itemMap = DataBaseHelper.queryForMap(sql, cjMap.get("ypbwm"));
					if(itemMap==null){
						InsZsBaYbItem item = new InsZsBaYbItem();
						//item.setBgsj(bgsj); //变更时间
						item.setPkHp("fcace0e12aba439991c0eb001eef02f3"); //医保类别
						item.setXmbh(map.get("sdm").toString());//项目编号
						item.setXmlb(zfkMap.get("xmlb").toString());//项目类别 1 药品，2 材料，3 诊疗
						item.setZwmc(cjMap.get("cpmc").toString());//中文名称
						//item.setYwmc("");//英文名称
						item.setSpcode(hzToSzmdx(cjMap.get("cpmc").toString()));//拼音码
						item.setdCode(getWBCode(cjMap.get("cpmc").toString()));//其他码
						item.setFldm(zfkMap.get("y18xfldm")==null?"":zfkMap.get("y18xfldm").toString());//分类代码
						item.setYpgg(cjMap.get("gg")==null?"":cjMap.get("gg").toString());//规格
						item.setYpjx(cjMap.get("jx")==null?"":cjMap.get("jx").toString());//剂型
						//item.setZjm("");//助记码
						item.setYwmldj(zfkMap.get("ywmldj").toString());//药品目录登记 1甲类2乙类3自费
						item.setJbzfbl(new BigDecimal(zfkMap.get("jbylzfbl").toString()));//基本医疗自费比例
						item.setSqzfbl(new BigDecimal(zfkMap.get("sqmzzfbl").toString()));//社区自费比例
						...
						//item.setCjlbz();//产检库标志
						//item.setBzdm();//病种代码
						item.setYjjypbm(cjMap.get("ypbwm").toString());//药监局药品编码
						item.setYyclzczmc(map.get("zcmc").toString());//医用材料的注册证产品名称
						item.setYyclsyjzch(cjMap.get("pzwh").toString());//医用材料的食药监注册号
						//item.setBz();//注册备注
						item.setScdw(cjMap.get("scqy").toString());//生产单位
						//item.setTxbz();//特项标志
						//item.setCwlr();//除外内容
						//item.setSm();//说明
						//item.setXmnh();//项目内涵
		...
		return "总数量："+list.size() + ",已有数据:"+yyNum+"条，成功导入："+cgNum+"条,成功数量:"+(cgNum+yyNum)+"条!";
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public String saveSdmlHcysk(String param,IUser user){
		
		String sql = "select top 10000 * from lsb_qgyb_hcysk where status is null";
		List<Map<String, Object>> list = DataBaseHelper.queryForList(sql);
		...
		for (Map<String, Object> map : list) {
			sql = "select * from lsb_zfk where xmdm = ? ";
			Map<String, Object> zfkMap = DataBaseHelper.queryForMap(sql, map.get("shcdm"));
			if(zfkMap!=null){
				InsZsBaYbItem item = DataBaseHelper.queryForBean("select * from ins_item where xmbh = ? and yyclsyjzch = ? and DJCPMC = ? and GGXHS = ? "
						+ "and scdw = ? and ybzfbz = ? and  yjfl = ? and ejfl = ? and sjfl = ? and cz = ? and tz = ?",
						InsZsBaYbItem.class, map.get("shcdm"), map.get("zczh"), map.get("djcpmc"), map.get("ggxhs"), map.get("hcqy"),
						map.get("ybzfbz"),map.get("yjfl"),map.get("ejfl"),map.get("sjfl"),map.get("cz"),map.get("tz"));
				if(item==null){
					item = new InsZsBaYbItem();
					//item.setBgsj(bgsj); //变更时间
					item.setPkHp("fcace0e12aba439991c0eb001eef02f3"); //医保类别
					item.setXmbh(map.get("shcdm").toString());//项目编号
					item.setXmlb("2");//项目类别 1 药品，2 材料，3 诊疗
					item.setZwmc(map.get("shczw").toString());//中文名称
					//item.setYwmc("");//英文名称
					item.setSpcode(hzToSzmdx(map.get("shczw").toString()));//拼音码
					item.setdCode(getWBCode(map.get("shczw").toString()));//其他码
					item.setFldm(zfkMap.get("y18xfldm")==null?"":zfkMap.get("y18xfldm").toString());//分类代码
					//item.setYpgg(map.get("zcgg")==null?"":map.get("zcgg").toString());//规格
					//item.setYpjx(map.get("zcjx")==null?"":map.get("zcjx").toString());//剂型
					//item.setZjm("");//助记码
					item.setYwmldj(zfkMap.get("ywmldj")==null?null:zfkMap.get("ywmldj").toString());//药品目录登记 1甲类2乙类3自费
					item.setJbzfbl(new BigDecimal(zfkMap.get("jbylzfbl").toString()));//基本医疗自费比例
					item.setSqzfbl(new BigDecimal(zfkMap.get("sqmzzfbl").toString()));//社区自费比例
					item.setLxzfbl(new BigDecimal(zfkMap.get("lxzfbl").toString()));//离休自费比例
					//item.setGszfbl();//工伤自费比例
					...
					//item.setTmzfbl();//特定门诊自费比例
					//item.setBczfbl();//补充医疗自费比例
					item.setJzjg(zfkMap.get("ypmldj")==null?null:zfkMap.get("ypmldj").toString()==""?null:new BigDecimal(zfkMap.get("ypmldj").toString()));//基准价格
					item.setXjje(zfkMap.get("xjje")==null?null:zfkMap.get("xjje").toString()==""?null:new BigDecimal(zfkMap.get("xjje").toString()));//限价金额
					item.setLxxjje(zfkMap.get("lxxjje")==null?null:zfkMap.get("lxxjje").toString()==""?null:new BigDecimal(zfkMap.get("lxxjje").toString()));//离休限价金额
					//item.setJzkbz();//基准库标志
					//item.setXzkbz();//限制库标志
					//item.setTdkbz();//特定库标志
					//item.setCjlbz();//产检库标志
					...
					//item.setYjjypbm(map.get("ypbwm").toString());//药监局药品编码
					//item.setYyclzczmc(map.get("zcmc").toString());//医用材料的注册证产品名称
					item.setYyclsyjzch(map.get("zczh").toString());//医用材料的食药监注册号
					//item.setBz();//注册备注
					item.setScdw(map.get("hcqy").toString());//生产单位
					//item.setTxbz();//特项标志
					//item.setCwlr();//除外内容
					//item.setSm();//说明
					//item.setXmnh();//项目内涵
					//item.setKssj();//开始时间
					//item.setZssj(zssj);//终止时间
					item.setNationCode(map.get("ybhcdm").toString());
					item.setYbzfbz(map.get("ybzfbz")==null?null:map.get("ybzfbz").toString());
					item.setYjfl(map.get("yjfl")==null?null:map.get("yjfl").toString());
					item.setEjfl(map.get("ejfl")==null?null:map.get("ejfl").toString());
					item.setSjfl(map.get("sjfl")==null?null:map.get("sjfl").toString());
					item.setCz(map.get("cz")==null?null:map.get("cz").toString());
					item.setTz(map.get("tz")==null?null:map.get("tz").toString());
					item.setDjcpmc(map.get("djcpmc")==null?null:map.get("djcpmc").toString());
					item.setGgxhs(map.get("ggxhs")==null?null:map.get("ggxhs").toString());
					DataBaseHelper.insertBean(item);
					addNum ++;
					System.out.println((addNum+upNum)+"");
				}
				else{
					if(!item.getNationCode().equals(map.get("ybhcdm").toString())){
						msg += item.getNationCode()+"|"+map.get("ybhcdm")+"|"+map.get("shcdm")+"|"+map.get("shczw")+"|"+map.get("ybzfbz")+"|"
								+map.get("yjfl")+"|"+map.get("ejfl")+"|"+map.get("ybtym")+"|"+map.get("cz")+"|"
								+map.get("tz")+"|"+map.get("zczh")+"|"+map.get("djcpmc")+"|"+map.get("hcqy")+"|"
								+map.get("ggxhs")+"|"+map.get("sjfl")+";";
						item.setBgsj(new Date()); //变更时间
						item.setNationCode(map.get("ybhcdm").toString());
						DataBaseHelper.updateBeanByPk(item);
						upNum ++;
			...
			DataBaseHelper.execute("update lsb_qgyb_hcysk set status = '1' where ybhcdm = ? and shcdm = ? and zczh = ? and djcpmc = ? and hcqy = ? and ggxhs = ?",
					map.get("ybhcdm"), map.get("shcdm"), map.get("zczh"), map.get("djcpmc"), map.get("hcqy"), map.get("ggxhs"));
			dqNum ++;
			System.out.println(msg);
			System.out.println("检查条数："+dqNum+",导入条数："+addNum+",修改条数："+upNum);
		}
		System.out.println("总数量："+list.size() + ",导入："+addNum+"条,修改："+upNum+"条,成功数量："+(addNum+upNum)+"!");
		return "总数量："+list.size() + ",导入："+addNum+"条,修改："+upNum+"条,成功数量："+(addNum+upNum)+"!";
	}
	...
	public static String hzToSzmdx(String name){
		String szmdx = "";
		char b;
		int a=name.length();
		for(int k=0;k<a;k++){

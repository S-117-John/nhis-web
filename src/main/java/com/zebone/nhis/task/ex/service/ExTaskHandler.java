...
import com.zebone.nhis.task.ex.dao.OrderExecAndCgListMapper;
 ...
 */
@Service
public class ExTaskHandler {
	private Logger logger = LoggerFactory.getLogger("nhis.quartz");
	@Resource
	private OrderExecAndCgListMapper orderExecAndCgListMapper;
	 @Resource
    ...
    private StringBuilder execCg(String pkOrg, String pkPv, StringBuilder errMsg, String nameDeptNs, String bedNo, String namePi,String pkDeptNs,QrtzJobCfg cfg){
		 ...
		 String isDatePlan="";
		 String isOnlyExecLongTermOrd = "";//为1表示只自动执行长期医嘱
		 if(!CommonUtils.isEmptyString(cfg.getJobparam())){
			...
			if(jobParam!=null){
				...
				isOnlyExecLongTermOrd= jobParam.get("isOnlyExecLongTermOrd")!=null ? jobParam.get("isOnlyExecLongTermOrd").toString() : "";
			}
		 }
		 map.put("isOnlyExecLongTermOrd", isOnlyExecLongTermOrd);
			
		 List<ExlistPubVo> result = new ArrayList<ExlistPubVo>();
		 //查询执行科室为本病区护理和治疗类医嘱的未执行执行单
		 result =  orderExecAndCgListMapper.queryExecListByCon(map);

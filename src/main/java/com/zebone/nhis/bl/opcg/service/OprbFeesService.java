...
public class OprbFeesService {
	...
	public List<Map<String, Object>> querySettledFeesToReBack(String param, IUser user) {
		...
		mapParam.put("flagPrepReback",oprbFeesDto.getFlagPrepReback());
...
//		double patientsPayRefund = twoModeFee.getPatientsPay(); // 负值！要退给患者自付的金额
	...
	/**
	 * 修改患者费用明细金额（体检打折） 交易号： 007002003049
	 *
	 * @param param
	 * @param user
	 * @return
		...
		if (discount > 0 && discount <= 1) {
			...
			if (opdtList != null && opdtList.size() > 0) {
				...
				for (BlOpDt opdt : opdtList) {
					...
					opdt.setAmount(BigDecimal.valueOf(MathUtils.mul(opdt.getPrice(), opdt.getQuan())).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue());
					opdt.setAmountHppi(BigDecimal.valueOf(MathUtils.mul(opdt.getPrice(), opdt.getQuan())).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue());
					opdt.setAmountPi(BigDecimal.valueOf(MathUtils.mul(opdt.getPrice(), opdt.getQuan())).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue());

					amount = amount.add(BigDecimal.valueOf(opdt.getAmountPi()));
					amountOld = amountOld.add(BigDecimal.valueOf(MathUtils.mul(opdt.getPriceOrg(), opdt.getQuan())));
				}

				amount = amount.setScale(2,BigDecimal.ROUND_HALF_UP);
				amountOld = amountOld.setScale(2,BigDecimal.ROUND_HALF_UP);

				amtCz = amountOld.multiply(BigDecimal.valueOf(discount)).subtract(amount).setScale(2,

...
import com.zebone.nhis.bl.opcg.dao.OpcgOrdInsteadEntryMapper;
import com.zebone.nhis.bl.opcg.vo.OpCgCnOrder;
import com.zebone.nhis.bl.opcg.vo.OpOrdInsteadEntryDto;
 ...
 */
@Service
public class OpcgOrdInsteadEntryService {
	...
	@Autowired
	private OpcgOrdInsteadEntryMapper opcgOrdInsteadEntryMapper;
	/**
	 * 根据患者主键查询患者的就诊记录
	 * @param param
	 * @param user
	...
	public void savePresAndFees(String param, IUser user) {
		...
		if (ordListParam.size() <= 0 && !"1".equals(ooied.getSaveFlag())) throw new BusException("请录入处方明细!");
        ...
        if(ordListParam.isEmpty()) return;
		...
		//查询当前医生考勤科室
		String pkEmpOrd = ordListParam.get(0).getPkEmpOrd();
		String pkDeptJob  = getPkDeptJob(pkEmpOrd);
		// 组装数据
	...
	public String saveHerbAndFees(String param, IUser user) {
		...
		//查询当前医生考勤科室
		String pkDeptJob = getPkDeptJob(presInfo.getPkEmpOrd());
	...
	/**
	 * 根据患者就诊主键,查询录入人是当前登录人的处方明细
	 * @param param
	 * @param user
	 * @return
	 */
	public List<Map<String, Object>> queryCurEmpPresInfo(String param, IUser user) {
		Map paramMap = JsonUtil.readValue(param, Map.class);
		User u = (User) user;
		paramMap.put("pkEmp",u.getPkEmp());
		List<Map<String, Object>> result = opcgOrdInsteadEntryMapper.queryCurEnterPresInfo(paramMap);
//		if (result == null || result.size() == 0)
//			throw new BusException("此处方不是当前登录用户录入的，不能修改");
		return result;
	}
	...
	/**
	 * 根据患者就诊主键,查询录入人是当前登录人的检查检验治疗
	 * @param param
	 * @param user
	 * @return
	 */
	public List<Map<String, Object>> queryCurEmpLisRisInfo(String param, IUser user) {
		Map paramMap = JsonUtil.readValue(param, Map.class);
		String pkPv = CommonUtils.getString(paramMap.get("pkPv"));
		String codeOrdtype = CommonUtils.getString(paramMap.get("codeOrdtype"));
		if(CommonUtils.isEmptyString(codeOrdtype)&&StringUtils.isBlank(pkPv)){
			throw new BusException("您选择的是非医嘱项目，不能修改！");
		}
		User u = (User) user;
		paramMap.put("pkEmp",u.getPkEmp());
		List<Map<String, Object>> result = opcgOrdInsteadEntryMapper.queryCurEnterLisRisInfo(paramMap);
//		if (result == null || result.size() == 0)
//			throw new BusException("此医嘱不是当前登录用户录入的，不能修改！");
		return result;
	...
	/**
	 * 根据患者就诊主键,查询录入人是当前登录人的草药明细信息
	 * @param param
	 * @param user
	 * @return
	 */
	public List<Map<String, Object>> queryCurEmpHerbInfo(String param, IUser user) {
		Map paramMap = JsonUtil.readValue(param, Map.class);
		User u = (User) user;
		paramMap.put("pkEmp",u.getPkEmp());
		List<Map<String, Object>> result = opcgOrdInsteadEntryMapper.queryCurEnterHerbInfo(paramMap);
//		if (result == null || result.size() == 0)
//			throw new BusException("此处方不是当前登录用户录入的，不能修改");
		return result;
	}
	 ...
	 * @return
	 */
	public List<Map<String,Object>> ExtendedAttributes(String param, IUser user){
		Map<String, Object> paramMap = JsonUtil.readValue(param, Map.class);
		Map<String, Object> map = opcgOrdInsteadEntryMapper.MasterPlan(paramMap);
		List<Map<String,Object>> ValAttr = opcgOrdInsteadEntryMapper.ExtendedAttributes(map.get("pkInsu").toString());
		return ValAttr;
	}
	...
	public List<Map<String,Object>> queryDictAttrByType(String param,IUser user){
		Map<String,Object> paramMap = JsonUtil.readValue(param, Map.class);
		List<Map<String,Object>> list = opcgOrdInsteadEntryMapper.queryDictAttrByType(paramMap);
		return list;
	}

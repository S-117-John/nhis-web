...
@Service
public class CgQryMaintainService {
	 ...
	 * @throws BusException
	 */
	public BdPayer qryBdPayerByName(Map<String, Object> mapParam) throws BusException {

		String sql = "select * from BD_PAYER where name=?  and (del_flag = '0' or del_flag is null) ";
		BdPayer bdPayer = DataBaseHelper.queryForBean(sql, BdPayer.class, new Object[] { mapParam.get("name") });
		return bdPayer;
	}
	/**
	 * 根据医保计划主键批量查询医保信息
...
//		sql.append(" and price.pk_org = ? inner join bd_ord_item boi on item.pk_item = boi.pk_item  inner join bd_pricetype_cfg cfg on price.eu_pricetype=cfg.eu_pricetype and cfg.pk_hp=? and cfg.pk_org=? where item.pk_item = ? and ");
	...
	/**
	 * 根据费用分类主键查询核算码
	 * @param {pkItemcates(费用分类主键集合),euType,pkOrg}
	 * @return
	 * @throws BusException
	 */
	...
	public List<Map<String, Object>> queryBlCgIpDetails(Map<String, Object> paramMap) {
		...
		if(paramMap.containsKey("highValueBarcodeFlag"))
		{
			String highValueBarcodeFlag = (String)paramMap.get("highValueBarcodeFlag");
			if("1".equals(highValueBarcodeFlag))
			{
				lstRet = cgQryMaintainMapper.queryBlCgIpDetailsIncludeHVB(paramMap);
			}
			else
			{
				lstRet = cgQryMaintainMapper.queryBlCgIpDetails(paramMap);
			}
		}
		else
		{

...
/**
 * @Classname ZsbaCgLabService
 * @Description 检验记费博爱个性化实现
 * @Date 2021-05-29 16:55
 * @Created by wuqiang
 */
@Service("ZsbaCgLabService")
public class ZsbaCgLabService implements ICgLabService {
    @Resource
        ...
        if (blCgPubParamVos!=null && blCgPubParamVos.size()>0) {
            ...
            if(sampList!=null && sampList.size()>0){
                ...
                if(combList!=null && combList.size()>0){
                    cgProcessHandler.hbOrdInfoBycodeCombAndFlagAddcol(blCgPubParamVos,sampList,combList);
                }
            }
        }
    }
    ...
    public List<ItemPriceVo> filterOrdItem(List<ItemPriceVo> priceList, List<ItemPriceVo> itemList) {
        ...
        itemList.forEach(item ->{
        	pkCnords.add(item.getPkCnord());
        });
        List<Map<String, Object>> labList = cgStrategyPubMapper.qryOrdLabApplyListInfo(pkCnords);
        
        for (ItemPriceVo vo : itemList) {
        	if(null != labList && !labList.isEmpty()) {
        		labList.forEach(lab ->{
        			if(vo.getPkCnord().equals(MapUtils.getObject(lab, "pkCnord"))) {
        				vo.setCodeOrdtype(MapUtils.getString(lab, "codeOrdtype"));
        				vo.setDateStart(DateUtils.strToDate(MapUtils.getString(lab, "dateStart","")));
        				vo.setDtSamptype(MapUtils.getString(lab, "dtSamptype"));
        			}
        		});
        	}
            //校验是否是检验医嘱

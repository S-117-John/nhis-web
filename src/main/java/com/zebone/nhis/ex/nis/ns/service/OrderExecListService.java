...
import com.zebone.nhis.ex.nis.ns.vo.ExCgVo;
 ...
 */
@Service
public class OrderExecListService {
	@Resource
	private OrderExecListMapper orderExListMapper;
	@Resource
...
//    		if("1".equals(ApplicationUtils.getSysparam("CN0028", false))){
//    			//删除未执行的执行单
//    			orderExListMapper.deleteExList(paramMap);
//        	}
    	}
    ...
    public String processStopOrd(List<OrderCheckVo> stop_obj_list,Map<String,Object> paramMap,User user){
    	...
    	//本病区已执行的非药品医嘱执行单列表
    	List<ExCgVo> exlist = new ArrayList<ExCgVo>();
    	//非本病区已执行的非药品医嘱执行单列表
      ...
      for(OrderCheckVo stopvo : stop_obj_list){
    	...
    	//计算医嘱频次周期次数
		List<FreqTimeVo> timelist = orderExListMapper.queryFreqTimeList(stopvo.getCodeFreq());
    	...
    	if(stopvo.getLastNum()==null){//无末次
    		...
    		if(stopvo.getLastNum().intValue()>stopvo.getFirstNum().intValue()){
    			...
    			//无执行时刻，则执行单全删
    			if(!"3".equals(stopvo.getEuExtime())&&(timelist == null || timelist.size()<=0)){
    					pk_nolist.add(stopvo.getPkCnord());
    			}else {
					...
					if (stopDayOccList == null || stopDayOccList.size() <= 0 || stopDayOccList.size() < stopvo.getLastNum().intValue()) {
						...
						//指定间隔模式，取停止日期符合末次数量的最后一条执行单执行时刻作为删除执行单起始点
						if ("3".equals(stopvo.getEuExtime())) {
							if (stopDayOccList.get(index - 1) != null) {
								Date datePlan = stopDayOccList.get(index - 1).getDatePlan();
								beginTime = DateUtils.getDateTimeStr(datePlan);
							}
						} else if (timelist.size() >= stopvo.getLastNum()) {
							FreqTimeVo timevo = timelist.get(index - 1);
...
//    	}
     ...
     * @return
     */
    private Map<String,Object> sendOccListToPlat(List<String> del_list,List<ExCgVo> exlist,List<String> update_list,List<String> update_other_list){
    	//启用平台标志
     ...
     * @return
     */
    private boolean isStopNumValid(OrderCheckVo vo,List<FreqTimeVo> timelist){ 
		...
		if(flagWeek){//按周执行，判断周数是否与当日一致
			...
			for(FreqTimeVo freqtime : timelist){
     ...
     * @param u
     */
	private void rtnCgAndCancelEx(List<ExCgVo> exlist,User u){
		List<RefundVo> rtnList = new ArrayList<RefundVo>();
		for(ExCgVo exvo:exlist){
	...
	private void createFirstExecList(List<OrderCheckVo> checklist) throws ParseException{
    	...
    	for(OrderCheckVo vo:checklist){
    		...
    		//计算医嘱频次周期次数
			List<FreqTimeVo> timelist = orderExListMapper.queryFreqTimeList(vo.getCodeFreq());
			boolean flagWeek = "1".equals(vo.getEuCycle());
			List<FreqTimeVo> timeWeeklist = null;//按周执行当天需要执行的频次时刻集合
			...
			//频次按执行周期当天本应该无执行单的，首日不生效
    		if(flagWeek){//按周执行，判断周数是否与当日一致
    			timeWeeklist = new ArrayList<FreqTimeVo>();
    			boolean sameFlag = false;
    			for(FreqTimeVo freqtime : timelist){
	 ...
	 * 处理当天开当天停医嘱执行
	 */
	private void processStopNumWhenSameDate(User user,List<String> update_other_list,List<String> del_list,List<ExCgVo> exlist,OrderCheckVo stopvo,String currTime){
		...
		for(int i=stopvo.getLastNum().intValue();i<allOccList.size();i++){
			...
			if("0".equals(occvo.getEuStatus())){
				...
				if((user.getPkDept().equals(stopvo.getPkDeptExec())&&"0".equals(stopvo.getFlagDurg()))||"1".equals(stopvo.getFlagDurg())&&"1".equals(stopvo.getFlagBase())){
					//执行科室为本病区的非药品或者基数药查询
					List<ExCgVo> cglist = orderExListMapper.queryExAndCglist(occvo.getPkExocc());

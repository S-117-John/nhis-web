...
import com.zebone.nhis.scm.material.dao.MtlPuOrderMapper;
 ...
 */
@Service
public class MtlPuOrderService {

	@Resource
	private MtlPuOrderMapper mtlPuOrdMapper;
	...
	public List<Map<String, Object>> queryPuPlan(String param, IUser user) {
		...
		List<Map<String, Object>> list = mtlPuOrdMapper.queryPuPlanByCon(map);
		if (list != null && list.size() > 0) {
			for (Map<String, Object> result_map : list) {
				result_map.put("dtlist", mtlPuOrdMapper.queryPuPlanDtByCon(result_map));
			}
		}
		return list;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<MtlPdPlanDtInfo> queryPuPlanDt(String param, IUser user) {
		Map<String, Object> map = JsonUtil.readValue(param, Map.class);
		return mtlPuOrdMapper.queryPuPlanDtByCon(map);
	}
	...
	public void rtnPuPlan(String param, IUser user) {
		...
		String sql = "update pd_plan set eu_status='0', " 
				+ "ts = to_date('" + DateUtils.getDate("yyyyMMddHHmmss") + "','YYYYMMDDHH24MISS') "
				+ "where pk_pdplan=? and eu_status='1'";
	...
	public void updatePuPlanDt(String param, IUser user) {
		List<PdPlanDtVo> dtlist = JsonUtil.readValue(param,
				new TypeReference<List<PdPlanDtVo>>() {
				});
		if (dtlist == null || dtlist.size() <= 0)
			return;
		String[] sqls = new String[dtlist.size()];
		for (int i = 0; i < dtlist.size(); i++) {
			sqls[i] = "update pd_plan_detail set quan_pack = " + dtlist.get(i).getQuanPack() 
					+ ", quan_min = " + dtlist.get(i).getQuanPack() + " * pack_size "
					+ ", ts = to_date('" + DateUtils.getDate("yyyyMMddHHmmss") + "','YYYYMMDDHH24MISS') " 
	...
	public PuOrderVo savePuOrder(String param, IUser user) {
		...
		if (vo == null || vo.getDt().size() <= 0)
			throw new BusException("未获取到需要保存的数据");
		PdPurchase order = new PdPurchase();
		ApplicationUtils.copyProperties(order, vo);
		List<PuOrderDtVo> list = vo.getDt();//待保存的订单列表
		// 修改的保存
		if (!CommonUtils.isEmptyString(vo.getPkPdpu())) {
			...
			for (PuOrderDtVo dtvo : list) {
				...
				dtvo.setQuanMin(MathUtils.mul(dtvo.getQuanPack(), (double)dtvo.getPackSize()));
		...
		if (vo.getDelDtList() != null && vo.getDelDtList().size() > 0) {
			DataBaseHelper.getJdbcTemplate().batchUpdate( "delete from pd_purchase_dt "
					+ " where pk_pdpudt = ? ", vo.getDelDtList());
		}
	...
	public PuOrderVo approvePuOrder(String param, IUser user) {
		...
		// 更新采购订单信息
		String update_pu_sql = "update pd_purchase set eu_status='1',flag_chk='1',"
				+ " pk_emp_chk=:pkEmp,name_emp_chk=:nameEmp,date_chk=:dateChk,"
				+ " ts=:dateChk where pk_pdpu = :pkPdpu";
	...
	public void deletePuOrder(String param, IUser user) {
		String pkPdPu = JsonUtil.readValue(param, String.class);
		if (CommonUtils.isEmptyString(pkPdPu))
			throw new BusException("未获取到删除订单的相关入参！");
		String delete_dt_sql = "delete from pd_purchase_dt  "
				+ "where pd_purchase_dt.pk_pdpu=? and  exists (select 1 from pd_purchase  "
				+ "where pd_purchase_dt.pk_pdpu=pd_purchase.pk_pdpu and pd_purchase.eu_status='0')";
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<MtlPuOrdInfo> queryPuOrder(String param, IUser user) {
		Map<String, Object> map = JsonUtil.readValue(param, Map.class);
		...
		return mtlPuOrdMapper.queryPuOrderList(map);
	}
	 ...
	 * @return
	 */
	public List<MtlPuOrdDtInfo> queryPuOrderDt(String param, IUser user) {
		String pk_pdpu = JsonUtil.readValue(param, String.class);
		return mtlPuOrdMapper.queryPuOrderDtList(pk_pdpu);
	}

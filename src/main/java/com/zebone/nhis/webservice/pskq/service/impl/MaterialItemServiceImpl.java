...
import com.zebone.nhis.webservice.pskq.model.MaterialItem;
...
import com.zebone.nhis.webservice.pskq.service.MaterialItemService;
...
public class MaterialItemServiceImpl implements MaterialItemService {
    ...
    public void save(String param, ResultListener listener) {
        try {
            ...
            List<DataElement> list = (List<DataElement>) requestBody.getMessage().get("MATERIAL_PROPERTY");
            MaterialItem materialItem = (MaterialItem) MessageFactory.deserialization(list, new MaterialItem());
            JSONObject json1 =JSONObject.fromObject(materialItem);
            BdOuEmployee bdOuEmployee= JSON.parseObject(JSON.toJSONString(json1), BdOuEmployee.class);

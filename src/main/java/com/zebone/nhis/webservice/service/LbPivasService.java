...
import com.zebone.nhis.webservice.dao.LbPivasDrugMapper;
import com.zebone.nhis.webservice.support.RespJson;
import com.zebone.nhis.webservice.vo.lbpivas.BlCgVo;
import com.zebone.nhis.webservice.vo.lbpivas.BlIpCgVo;
import com.zebone.nhis.webservice.vo.lbpivas.BlPubParamVo;
 ...
 */
@Service
public class LbPivasService {

    @Resource
    private LbPivasDrugMapper lbPivasDrugMapper;
    /**
     * 灵璧静配发药
     */
    public void saveIpPivasDrug(Map<String,Object> paramMap,String codeEmp){
        for (String code:paramMap.keySet()) {
            ...
            List<Map<String,Object>> pivasDrugList=lbPivasDrugMapper.qryPivaDrugDtInfo(qryMap);
            if(pivasDrugList==null || pivasDrugList.size()==0){
                throw new BusException("未查询到待发药的明细数据，请核对！");
            }
            ...
            ResponseJson response =apputil.execService("SCM", "IpPdDeDrugService", "mergeIpDeDrug", pivasDrugList,user);
     ...
     * 灵璧静配退药
     */
    public void saveIpRetPivasDrug(Map<String,Object> paramMap,String codeEmp){
        for (String code:paramMap.keySet()) {
            String sql="select dept.pk_dept,dept.pk_org,sto.pk_store from bd_ou_store sto inner join bd_ou_dept dept on dept.pk_dept=sto.pk_dept where dept.code_dept=?";
            Map<String,Object> deptMap= DataBaseHelper.queryForMap(sql,new Object[]{code});
            ...
            List<Map<String,Object>> pivasDrugList=lbPivasDrugMapper.qryPivaRetDrugDtInfo(qryMap);
            if(pivasDrugList==null || pivasDrugList.size()==0){
                throw new BusException("未查询到待退药的明细数据，请核对！");
            }
